...
feature
	...
	--t_head_list_does_not_copy_changes
			...
			create r1
			t1.append (a_value)
			t1.append (other_element_1)
			t1.append (other_element_2)
			t1.append (other_element_1)
			t1.append (other_element_2)
			r1 := t1.head_list (3)
			t1.prepend (other_element_1)
			assert ("Il primo elemento della testa è stato modificato", r1.first_element /= Void and then attached r1.first_element as fe implies fe.value = a_value)
	...
	--t_head_list_does_not_change_original
			...
			create r2
			t2.append (a_value)
			t2.append (other_element_1)
			t2.append (other_element_2)
			t2.append (other_element_1)
			t2.append (other_element_2)
			r2 := t2.head_list (3)
			r2.prepend (other_element_1)
			assert ("Il primo elemento dell'originale è stato modificato", t2.first_element /= Void and then attached t2.first_element as fe implies fe.value = a_value)
		end
...
--	t_tail_list_does_not_copy_changes
			...
			r1 := t1.tail_list (3)
			t1.append (a_value)
			assert ("L'ultimo elemento della coda è stato modificato", r1.last_element /= Void and then attached r1.last_element as le implies le.value = a_target)
...
--	t_tail_list_does_not_change_original
			...
			r2 := t2.tail_list (3)
			r2.append (a_value)
			assert ("L'ultimo elemento dell'originale è stato modificato", t2.last_element /= Void and then attached t2.last_element as le implies le.value = a_target)
		end
